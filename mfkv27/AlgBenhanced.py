...
class Tour():
    """
    Stores information linked to each tour
...
# Simulated Annealing Algorithm Methods
class SA():
    def successor(self, T):
        ...
        # return successor tour
        return T

    def successor_enhanced(self, T):
        '''
        ==== 2-OPT METHOD ==== 

        Returns a "successor" using the 2-opt method:
        https://en.wikipedia.org/wiki/2-opt

        Enhanced version of generating a "successor" tour.
        '''
        # choose random i and k
        k = random.randint(2, len(T.tour)-1)
        i = random.randint(0, len(T.tour)-k)

        # 2-opt reversing method
        T.tour[i: (i + k)] = reversed(T.tour[i: (i + k)])
    ...
    def get_successor(self, current_tour, temp):
        '''
        Update the current tour if the new tour is shorter 
        or with the probability p = e^(delta/T)
        '''

        # generate the new tour
        new_tour = copy.deepcopy(current_tour)
        self.successor_enhanced(new_tour)

        # calculate the difference in the tour lengths
        delta = new_tour.length - current_tour.length

        # update the tour if tour is shorter (OR probability p)
        if delta < 0 or random.random() < math.exp(-delta/temp):
            current_tour = copy.deepcopy(new_tour)

        # return (potentially) updated tour
        return current_tour
...
def run_SA(temp_start, temp_end, alpha):
    ...
    # loop over schedule
    while temp > temp_end:
        # generate the successor tour
        succ = SA().get_successor(current, temp)
