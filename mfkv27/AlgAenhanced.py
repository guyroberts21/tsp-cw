...
tournament_size = 7  # used in tournament selection (parent selection method)
...
class GA:
    ...
    def next_generation(self, P):
        ...
        if elitism_size == 1:
            ...
            sorted_pop = sorted(
                P.pop, key=lambda x: x.length, reverse=False)
        ...
        for i in range(offset, newP.size):
            # select two parents
            parent1 = self.tournament_select(P)
            parent2 = self.tournament_select(P)
